...
export const commentResolvers = {
    ...
    Query: {
        commentByPost: (parent, {postId, first = 10, offset = 0}, {db, requestedFields}: {db: DbConnection, requestedFields: RequestedFields}, info: GraphQLResolveInfo) => {
            ...
            return db.sequelize.transaction((t: Transaction) => {
                return db.Comment
                    ...
                    .then((comment: CommentInstance) => {
                        ...
                        throwError(comment.get('user') != authUser.id, `Unathorized! You can only edit comments by yourself!`);
                        input.user = authUser.id;
        ...
        deleteComment: compose(...authResolvers)((parent, {id, input}, {db, authUser}: {db: DbConnection, authUser: AuthUser}, info: GraphQLResolveInfo) => {
            ...
            return db.sequelize.transaction((t: Transaction) => {
                return db.Comment
                    ...
                    .then((comment: CommentInstance) => {
                        throwError(!comment, `Comment with id ${id} not found!`);
                        throwError(comment.get('user') != authUser.id, `Unathorized! You can only delete comments by yourself!`);
